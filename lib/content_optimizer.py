...
class ContentOptimizer:
    ...
    def _calculate_priority(self, paragraph: str) -> int:
        ...
        # 关键词权重
        technical_keywords = [
            'api', 'function', 'class', 'method', 'algorithm', 'code', 'example',
            'implementation', 'solution', 'error', 'bug', 'performance', 'optimization',
            '算法', '函数', '方法', '实现', '解决方案', '代码', '示例', '性能', '优化'
        ]
        
        important_keywords = [
            'important', 'key', 'main', 'primary', 'core', 'essential', 'critical',
            '重要', '关键', '主要', '核心', '本质', '关键点'
        ]
        
        conclusion_keywords = [
            'conclusion', 'summary', 'result', 'finally', 'in short', 'overall',
            '总结', '结论', '最终', '综上', '总的来说'
        ]
        ...
        for chunk in chunks:
            if current_tokens + chunk.tokens <= target_tokens:
                ...
                if truncated_content:
                    selected_chunks.append(TextChunk(
    ...
    def _truncate_paragraph(self, paragraph: str, max_tokens: int) -> Optional[str]:
        """
        截断单个段落，保持句子完整性
        """
